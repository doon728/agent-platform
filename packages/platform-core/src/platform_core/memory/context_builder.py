...
def _read_enabled(memory_cfg: Dict[str, Any], memory_type: str) -> bool:
    """Check read_policies.<type>.enabled — defaults to True if not specified."""
    return ((memory_cfg.get("read_policies") or {}).get(memory_type) or {}).get("enabled", True)
...
def build_memory_context(
    ...
    read_short_term = _read_enabled(memory_cfg, "short_term")
    read_episodic = _read_enabled(memory_cfg, "episodic")
    read_semantic = _read_enabled(memory_cfg, "semantic")
    read_summary = _read_enabled(memory_cfg, "summary")
    ...
    for scope in scopes:
        ...
        if read_semantic and _is_top_level(domain, scope_type):
