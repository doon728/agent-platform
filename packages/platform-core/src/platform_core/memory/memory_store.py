...
class FileMemoryStore(MemoryBackend):
    ...
    def replace_memory(
        ...
        """Write a single memory record, replacing any existing records of the same type + fact_type."""
        metadata = metadata or {}
        ...
        if fact_type:
            # Targeted replace — only remove records with same memory_type + fact_type
            kept = [
                r for r in existing
                if not (
                    r.get("memory_type") == memory_type
                    and (r.get("metadata") or {}).get("fact_type") == fact_type
                )
            ]
        else:
            # Fallback — replace all of this memory_type
