...
def _get_episodic_cfg(ctx: Dict[str, Any]) -> Dict[str, Any]:
    """Extract episodic write config from ctx for write_locked enforcement."""
    memory_cfg = ctx.get("memory") or {}
    return ((memory_cfg.get("write_policies") or {}).get("episodic") or {})


def _get_domain(ctx: Dict[str, Any]) -> Dict[str, Any]:
    return ctx.get("domain") or {}


def _get_store(ctx: Dict[str, Any]):
    memory_cfg = ctx.get("memory") or {}
    return get_backend(memory_cfg)
...
def write_hitl_requested(
    ...
    write_episodic_event(
        ...
        episodic_cfg=_get_episodic_cfg(ctx),
...
def write_hitl_decision(
    ...
    if decision == "approved":
        ...
        metadata_extra = {"tool_result": str(tool_result) if tool_result else None}
    else:
        content = (
        ...
        metadata_extra = {}  # rejected — no tool result written
