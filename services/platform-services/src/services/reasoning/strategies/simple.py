...
# Simple reasoning strategy — single-pass: plan once → execute → respond.
# No loops, no retries, no multi-step reasoning.
# Default for Q&A, lookup, and conversational use cases.
...
from src.services.reasoning.planner import plan
    ...
    if matched:
        ...
        for s in scopes:
            ...
            if id_field and ctx.get(id_field):
                arg = ctx[id_field]
                break
        print(f"[hard_route] trigger='{matched.get('trigger')}' tool={tool} arg={arg}", flush=True)
...
def _planner(state: GraphState) -> GraphState:
    result = plan(state.get("prompt", "") or "", state.get("history") or [], state.get("ctx") or {})
...
def _executor(state: GraphState) -> GraphState:
    ...
    if isinstance(result, dict):
        ...
        router_trace = {"tool": tool, "resolved_input": tool_input, "mode": result.get("mode", "")}
        is_approval = result.get("result") in ("APPROVAL_REQUIRED", "NEEDS_HITL")
        ...
        executor_trace = {
            "tool": tool,
            "status": "hitl_required" if is_approval else ("success" if tool else ""),
...
def build_graph(checkpointer: Optional[object] = None):
    ...
    g.add_conditional_edges(
        "hard_route_check", _after_hard_route_check,
