...
export default function AssessmentView() {
  ...
  async function send() {
    ...
    try {
      ...
      const isApproval = outputAny?.result === "APPROVAL_REQUIRED" || outputAny?.answer?.result === "APPROVAL_REQUIRED";
      ...
      if (isApproval) {
        ...
        setMessages((m) => [...m, { id: uid(), role: "assistant", ts: Date.now(), text: "This action requires approval before it can be executed." }]);
        return;
      }
        ...
        <div style={{ flex: 1, padding: 12, border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, overflowY: "auto", background: "#0f172a" }}>
          ...
          {pendingApproval && (
            <div style={{ marginTop: 12, padding: 12, border: "1px solid rgba(245,158,11,0.6)", borderRadius: 10, background: "#3b2a11", color: "#f8fafc" }}>
              ...
              <div><b>Assessment:</b> {pendingApproval.tool_input?.assessment_id || "-"}</div>
              <div><b>Note:</b> {pendingApproval.tool_input?.note || "-"}</div>
              <div style={{ marginTop: 10 }}>
                <button className="btn" onClick={() => navigate("/supervisor")}>Open Approval Console</button>
              </div>
            </div>
          )}
          <div ref={bottomRef} />
