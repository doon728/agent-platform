...
from platform_core.observability.tracer import start_run, finish_run
...
class LangGraphRunner:
    ...
    def _ensure_app(self):
        if self._app is None:
            if self._build_graph_fn is None:
                raise ValueError("LangGraphRunner requires a build_graph_fn")
    ...
    def run(self, prompt: str, ctx: Dict[str, Any]) -> Any:
        ...
        for key in ("short_term", "episodic", "summary", "semantic",
                    "read_short_term", "read_episodic", "read_summary", "read_semantic"):
            if key in override:
        ...
        if pre_graph_cfg.get("enabled", False):
            try:
                ...
                print(f"[pre_graph_rag] chunks={len(_chunks)}", flush=True)
        ...
        audit_metadata = {
            ...
            "reasoning_strategy": (ctx.get("reasoning") or {}).get("strategy", "simple"),
            "turn_id": run_id,
            "thread_id": thread_id,
            "tenant_id": tenant_id,
        }
        ...
        if summary_cfg.get("enabled", False):
            ...
            memory_trace["skipped"]["summary"] = {"reason": "policy_disabled"}
        ...
        if episodic_cfg.get("enabled", False) and memory_policy_state["episodic"]:
            ...
            if _is_tool_success:
                ...
                memory_trace["written"]["episodic"] = {"status": "written", "trigger": "tool_success", "tool": _tool_used}
            else:
                memory_trace["skipped"]["episodic"] = {"reason": f"trigger not met (tool={_tool_used or 'none'})"}
        else:
            memory_trace["skipped"]["episodic"] = {"reason": "policy_disabled"}
        ...
        if semantic_cfg.get("enabled", False) and memory_policy_state["semantic"]:
            ...
            memory_trace["written"]["semantic"] = {"status": "written", "scope": "member_or_user", "trigger": semantic_cfg.get("trigger")}
        else:
            memory_trace["skipped"]["semantic"] = {"reason": "policy_disabled"}
