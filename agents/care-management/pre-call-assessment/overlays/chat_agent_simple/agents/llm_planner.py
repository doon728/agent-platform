...
Use direct_answer when no tool is needed — general knowledge questions, greetings, or anything answerable from context alone.

Scope-to-tool mapping (use this when the user asks for a summary or info):
- Active member → use get_member_summary or get_member with member_id
- Active case → use get_case_summary with case_id
- Active assessment → use get_assessment_summary with assessment_id
Never call get_assessment_summary with a member_id — that is always wrong.
"""
    )
